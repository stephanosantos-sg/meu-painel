function ScreenCalendar() {
  ...
  function nav(dir) {
    ...
    if (view === 'day') d.setDate(d.getDate() + dir);else if (view === 'week') d.setDate(d.getDate() + dir * 7);else d.setMonth(d.getMonth() + dir);
    setBaseDate(d);
  }
  function goToday() {
  ...
  return React.createElement(React.Fragment, null, React.createElement(TopBar, {
    ...
    }, fmt), React.createElement("button", {
      className: "btn-ghost small",
      onClick: () => nav(1)
    ...
    }, ['day', 'week', 'month'].map(v => React.createElement("button", {
      key: v,
    ...
    tasks: tasks,
    catMap: catMap
  }), view === 'day' && React.createElement(DayView, {
    baseDate: baseDate,
    tasks: tasks,
    catMap: catMap
  })));
...
function MonthView({
  ...
  return React.createElement("div", {
    ...
    return React.createElement("div", {
    ...
    }, d), dayTasks.length > 0 && React.createElement("div", {
      style: {
        display: 'flex',
        gap: 2,
      ...
      return React.createElement("div", {
        ...
        style: {
          ...
          height: 5,
          borderRadius: '50%',
          background: done ? 'var(--ink-4)' : color
        }
      });
    }), dayTasks.length > 4 && React.createElement("div", {
      className: "mono",
      style: {
        fontSize: 7,
        color: 'var(--ink-3)'
      }
    }, "+", dayTasks.length - 4)), dayTasks.length > 0 && React.createElement("div", {
      className: "mono",
      style: {
        fontSize: 8,
        color: hasOverdue ? 'var(--neon-a)' : 'var(--ink-4)',
        marginTop: 2
      }
...
function DayView({
  ...
  return React.createElement("div", {
  ...
  }, dayFmt, isToday && ' · hoje'), React.createElement("h3", {
    className: "panel-title",
    style: {
      marginTop: 4,
      marginBottom: 18
    }
  }, dayTasks.length, " tarefas."), timed.length > 0 && React.createElement("div", {
    style: {
      marginBottom: 16
    }
  }, React.createElement("div", {
  ...
  }, "Com hor\xE1rio"), timed.map((item, i) => {
    const done = item.type === 'slot' ? Orbita.isSlotDone(item.task, ds, item.time) : Orbita.isTaskDone(item.task, ds);
    const cat = catMap[item.task.cat];
    const color = cat ? Orbita.resolveColor(cat.color) : 'var(--neon-c)';
    return React.createElement("div", {
      key: i,
      onClick: () => item.type === 'slot' ? toggleSlot(item.task.id, ds, item.time) : toggleTask(item.task.id, ds),
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 10,
    ...
    }, item.task.icon && React.createElement("span", {
      style: {
        marginRight: 4
      }
    }, item.task.icon), item.label || item.task.text));
  })), untimed.length > 0 && React.createElement("div", null, React.createElement("div", {
    className: "eyebrow",
    style: {
      marginBottom: 8
    }
  }, "Sem hor\xE1rio"), React.createElement("div", {
    className: "task-list"
  }, untimed.map(t => React.createElement(TaskItem, {
    key: t.id,
    task: t,
    dateCtx: ds,
    catMap: catMap
